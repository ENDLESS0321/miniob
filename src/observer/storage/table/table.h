...
public:
  ...
  /**
    * @brief 更新一条记录
    * @details 通过事务来更新一条记录，这里只是更新记录，不涉及索引的更新。
    * @param trx 事务
    * @param record 记录
    * @param attribute_name 字段名
    * @param values 字段值
    */
  RC update_record(Record &record, const char *attribute_name, Value *values);
...
private:
  ...
  RowRecordPageHandler *record_page_handler_ = nullptr;  /// 记录页面操作
